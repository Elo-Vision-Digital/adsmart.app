...
- 🔥 **HARD GATE FOR GEMINI (MEMORY LOCK)** 🔥: Under NO circumstances are you allowed to use Gemini's built-in "Planning Mode" artifacts (`implementation_plan.md`, `task.md`) as a substitute for the AdSmart Harness (`docs/specs/*`). You MUST run `bash scripts/harness/new-sprint.sh` and lock the `CONTRACT.md` before making a single code edit. If the user asks you to implement a feature, your VERY FIRST ACTION must be creating the sprint folder and negotiating the `SPEC.md`/`CONTRACT.md`.

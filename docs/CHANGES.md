...
**Status:** Tasks 1-14 of 20 complete. The server side, shared contracts, shadcn UI primitives, the date-range filter, all three cards (Revenue, Users, Integrations), the integrating `AdminDashboardPage` (with skeleton + error/retry banner), and the Dashboard tab in `AdminLayout` are in place; `App.tsx` routing change, i18n, and deploy are pending. **`getDashboardMetrics` is exported in `functions/src/index.ts` but NOT yet invoked from the client and NOT yet deployed.** Final docs sweep + AGENTS.md/CLAUDE.md/QA-CHECKLIST.md/DATA-MODEL.md/DEPLOYMENT.md updates land at Task 19 once the feature ships end-to-end.
